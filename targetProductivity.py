...
def mainEnter():
    ...
    durationTargetHour, durationTargetMin, totalDuration = determineTargetTime(
        hourTarget, minuteTarget)
    finalMinute, finalHour, suffix = makingClock(
        durationTargetHour, durationTargetMin, hours, minutes, lunchTime, accuracy)
    targetClockOut = getTargetClockOut(finalMinute, finalHour, suffix)
...
def determineWorkMin(treatmin, prodTarg):
    ...
    # Will round presentation values if last number is >= 5
    accuracy = accuracy[-1]
...
def determineTargetTime(hourTarget, minuteTarget):
...
def makingClock(durationTargetHour, durationTargetMin, hours, minutes, lunchTime, accuracy):
    ...
    return finalMinute, finalHour, suffix


def getTargetClockOut(finalMinute, finalHour, suffix):
